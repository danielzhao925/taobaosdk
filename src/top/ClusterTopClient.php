...
class ClusterTaobaoSdk extends TaobaoSdk {
	...
	public function __construct($appkey = "",$secretKey = ""){
		ClusterTaobaoSdk::$applicationVar = new ApplicationVar;
		$this->appkey = $appkey;
		$this->secretKey = $secretKey ;
		$saveConfig = ClusterTaobaoSdk::$applicationVar->getValue();

		if($saveConfig){
			$tmpConfig = $saveConfig['dnsconfig'];
			ClusterTaobaoSdk::$dnsconfig = $this->object_to_array($tmpConfig);
			unset($tmpConfig);

			ClusterTaobaoSdk::$syncDate = $saveConfig['syncDate'];
			if(!ClusterTaobaoSdk::$syncDate)
				ClusterTaobaoSdk::$syncDate = 0;
		}
	}

	public function __destruct(){
		if(ClusterTaobaoSdk::$dnsconfig && ClusterTaobaoSdk::$syncDate){
			ClusterTaobaoSdk::$applicationVar->setValue("dnsconfig",ClusterTaobaoSdk::$dnsconfig);
			ClusterTaobaoSdk::$applicationVar->setValue("syncDate",ClusterTaobaoSdk::$syncDate);
			ClusterTaobaoSdk::$applicationVar->write();
		}
	}
	...
	public function execute($request = null, $session = null,$bestUrl = null){
		...
		if($currentDate - ClusterTaobaoSdk::$syncDate > $syncDuration * 60){
			$httpdns = new HttpdnsGetRequest;
			ClusterTaobaoSdk::$dnsconfig = json_decode(parent::execute($httpdns,null,$bestUrl)->result);
			$syncDate = date('U');
			ClusterTaobaoSdk::$syncDate = $syncDate ;
		}
		return parent::execute($request,$session,$bestUrl);
	}

	private function getDnsConfigSyncDuration(){
		if(ClusterTaobaoSdk::$cfgDuration){
			return ClusterTaobaoSdk::$cfgDuration;
		}
		if(!ClusterTaobaoSdk::$dnsconfig){
			return ClusterTaobaoSdk::$cfgDuration;
		}
		$config = json_encode(ClusterTaobaoSdk::$dnsconfig);
		if(!$config){
			return ClusterTaobaoSdk::$cfgDuration;
		}
		$config = ClusterTaobaoSdk::$dnsconfig['config'];
		$duration = $config['interval'];
		ClusterTaobaoSdk::$cfgDuration = $duration;
	...
	private function getVipByEnv($comUrl,$currentEnv){
		...
		if(!$urlSchema)
			return null;
		if(!array_key_exists($currentEnv,ClusterTaobaoSdk::$dnsconfig['env']))
			return null;

		$hostList = ClusterTaobaoSdk::$dnsconfig['env'][$currentEnv];
		if(!$hostList)
			return null ;
	...
	private function getEnvByApiName($apiName,$session=""){
		$apiCfgArray = ClusterTaobaoSdk::$dnsconfig['api'];
		if($apiCfgArray){
	...
	private function getDeafultEnv(){
		return ClusterTaobaoSdk::$dnsconfig['config']['def_env'];
	}
